...
public class Parser {
	...
	public Parser(String dataFileLocation, int classVariableLoc,
			boolean dataImputation, boolean dataDiscretization, int idNumLoc,
			boolean dataRemoval) throws IOException {
		...
		try {
			...
			while ((currentLine = br.readLine()) != null) {
				...
				// NOTE: for these datasets, all id numbers are in the first
				// position
				...
				// if the classVariable is not at the end of the line, put it
				// there
				// NOTE: for these datasets, the class variable is either in the
				// first or last position
				if (classVariableLoc == 0) {
					String classV = currentLine.substring(0,
							currentLine.indexOf(','));
					String features = currentLine.substring(
							currentLine.indexOf(',') + 1, currentLine.length());

					// Frankenstein the line back together with the class
					// variable in the last location
		...
		// NOTE: We are testing Breast Cancer with imputation, and with removing
		// values
		if (dataRemoval == true) {
	...
	// this is used to send parsed/imputed/discretized/removed data back to
	// RunModels
